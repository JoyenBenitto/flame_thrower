...
# Feature wish list for `0.1.0`:
- [x] 4 Statically allocated Multi Queue buffers (`SAMQ`), which provide VCs(virtual channels) for incoming flits from north, south, east and west direction, also provides a
VC for associated processing element.
- [x] 4 x 4x1 crossbar interface, provide interface to connect and communicate with a mesh topology of routers.
- [ ] Round Robin Arbiter (No priority arbiteration added yet).
- [x] XY routing scheme (Route along the X direction and then Y as in this routing scheme the location of every router is known).
